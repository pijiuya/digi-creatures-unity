...
当前版本：`0.1.7`。目标兼容 Unity `2022.3 LTS` 到 Unity `6.x`；已在 Unity `6.4` 项目中验证，并针对 Unity `6.0` Windows `.unitypackage` 导入和编译做了兼容处理。
...
导出文件默认位于 `Builds/DigiCreaturesAgent-<版本号>.unitypackage`，例如 `Builds/DigiCreaturesAgent-0.1.7.unitypackage`。导入客户项目后菜单会出现在 `数字生物` 下。
