...
当前版本：`0.1.8`。目标兼容 Unity `2022.3 LTS` 到 Unity `6.x`；已在 Unity `6.4` 项目中验证，并针对 Unity `6.0` Windows `.unitypackage` 导入和编译做了兼容处理。

## 兼容状态

- 推荐使用 UPM Git 安装，Unity 会自动解析 AI Navigation、glTFast、Input System、UGUI/TextMeshPro 和内置模块。
- `.unitypackage` 也可用于客户交付；导入后会带一个轻量依赖安装器。如果 Unity 没有自动启动安装器，请手动点击 `DigiCreatures > Install Dependencies` 或 `数字生物 > 高级设置 > 安装依赖`。
- 不需要 Cinemachine。DigiPlace demo 使用插件自己的摄像机辅助逻辑，不再编译引用 Cinemachine 2/3 API。
- URP 不是硬依赖，Built-in、URP 或客户自定义渲染管线都可以继续使用。
...
导出文件默认位于 `Builds/DigiCreaturesAgent-<版本号>.unitypackage`，例如 `Builds/DigiCreaturesAgent-0.1.8.unitypackage`。导入客户项目后菜单会出现在 `数字生物` 下。

`0.1.3` 起，`.unitypackage` 会导入到 `Assets/DigiCreaturesAgent`，并包含独立依赖安装器；导入后会自动尝试安装 AI Navigation、glTFast、Input System 和 UGUI。也可以手动点击：
