...
当前版本：`0.1.4`。已在 Unity `6.4` 项目中验证，目标兼容 Unity `6.x`。
...
导出文件默认位于 `Builds/DigiCreaturesAgent-<版本号>.unitypackage`，例如 `Builds/DigiCreaturesAgent-0.1.4.unitypackage`。导入客户项目后菜单会出现在 `数字生物` 下。
